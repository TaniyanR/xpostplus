...
<style>
.rss-builder{display:grid;gap:20px}.rss-toolbar,.rss-template,.rss-history{background:#fff;border:1px solid #dcdcde;border-radius:8px;padding:18px}.rss-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.rss-toolbar button,.rss-card button,.rss-template button{padding:9px 14px;border:1px solid #2271b1;border-radius:5px;background:#2271b1;color:#fff;cursor:pointer}.rss-toolbar .secondary,.rss-card .secondary,.rss-template .secondary{background:#fff;color:#2271b1}.rss-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:16px}.rss-card{background:#fff;border:1px solid #dcdcde;border-radius:9px;overflow:hidden}.rss-card.selected{outline:3px solid #72aee6}.rss-card img{display:block;width:100%;aspect-ratio:16/9;object-fit:cover;background:#f0f0f1}.rss-card-body{padding:14px}.rss-card h3{margin:8px 0;font-size:17px;line-height:1.5}.rss-meta{font-size:12px;color:#646970}.rss-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.rss-template textarea{width:100%;min-height:180px;padding:12px;box-sizing:border-box;font-family:inherit}.rss-shortcodes{display:flex;gap:7px;flex-wrap:wrap;margin:10px 0}.rss-shortcodes code{padding:5px 7px;background:#f0f0f1;border-radius:4px;cursor:pointer}.rss-preview-list{display:grid;gap:14px}.rss-preview{display:grid;grid-template-columns:180px 1fr;gap:15px;background:#fff;border:1px solid #dcdcde;border-radius:8px;padding:14px}.rss-preview img{width:100%;aspect-ratio:16/9;object-fit:cover}.rss-preview textarea{width:100%;min-height:145px;box-sizing:border-box;padding:10px}.rss-history table{width:100%;border-collapse:collapse}.rss-history th,.rss-history td{padding:10px;border-bottom:1px solid #dcdcde;text-align:left}.rss-empty{padding:30px;text-align:center;background:#fff;border:1px dashed #b6b6b6;border-radius:8px;color:#646970}@media(max-width:700px){.rss-preview{grid-template-columns:1fr}.rss-preview img{max-width:100%}}
</style>

<section class="rss-builder">
  <div>
    <h1>RSS投稿作成（試作画面）</h1>
    <p>RSSから取り込んだ記事を選び、ショートコード式テンプレートでX投稿文をまとめて作成できます。</p>
  </div>

  <div class="rss-toolbar">
    <strong>RSSサイト：</strong>
    ...
    <button type="button" id="generate-selected">選択した記事を投稿用に加工</button>
  </div>

  <div class="rss-template">
    <h2>投稿レイアウト</h2>
    <p>下のショートコードをクリックするとテンプレートへ追加できます。</p>
...
{hashtags}</textarea>
      ...
      <?php foreach ($articles as $article): ?>
        <article class="rss-card" data-article='<?= e(json_encode($article, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)) ?>'>
          ...
          <div class="rss-card-body">
            <label><input type="checkbox" class="article-check"> 投稿候補に選択</label>
            ...
            <div class="rss-actions">
              ...
              <a href="<?= e($article['url']) ?>" target="_blank" rel="noopener">記事を確認</a>
            </div>
          </div>
        </article>
      <?php endforeach; ?>
  ...
  <div>
    <h2>作成結果</h2>
    <div id="preview-list" class="rss-preview-list"><div class="rss-empty">記事を選択して「投稿用に加工」を押してください。</div></div>
  </div>

  <div class="rss-history">
    <h2>ポスト履歴（この画面でコピーした履歴）</h2>
    <table>
...
(() => {
  ...
  function updateSelected(card) {
    card.classList.toggle('selected', card.querySelector('.article-check').checked);
  }

  function addHistory(article) {
    history.unshift({time: new Date().toLocaleString('ja-JP'), title: article.title, status: 'コピー済み'});
    historyBody.innerHTML = history.map(row => `<tr><td>${escapeHtml(row.time)}</td><td>${escapeHtml(row.title)}</td><td>${escapeHtml(row.status)}</td></tr>`).join('');
  }
  ...
  function buildPreview(article) {
    ...
    wrapper.querySelector('.copy-post').addEventListener('click', async () => {
      await navigator.clipboard.writeText(wrapper.querySelector('textarea').value);
      addHistory(article);
      wrapper.querySelector('.copy-post').textContent = 'コピーしました';
    });
    wrapper.querySelector('.copy-image').addEventListener('click', async () => {
      await navigator.clipboard.writeText(article.image_url);
      wrapper.querySelector('.copy-image').textContent = 'コピーしました';
    });
    wrapper.querySelector('.mark-posted').addEventListener('click', () => {
      history.unshift({time: new Date().toLocaleString('ja-JP'), title: article.title, status: '投稿済み'});
      historyBody.innerHTML = history.map(row => `<tr><td>${escapeHtml(row.time)}</td><td>${escapeHtml(row.title)}</td><td>${escapeHtml(row.status)}</td></tr>`).join('');
      wrapper.querySelector('.mark-posted').textContent = '投稿済み';
    });
    return wrapper;
  }
  ...
  function generate(list) {
    ...
    if (!list.length) {
      previewList.innerHTML = '<div class="rss-empty">記事が選択されていません。</div>';
      return;
    }
    list.forEach(article => previewList.appendChild(buildPreview(article)));
  }

  cards.forEach(card => {
    card.querySelector('.article-check').addEventListener('change', () => updateSelected(card));
    card.querySelector('.generate-one').addEventListener('click', () => generate([articleOf(card)]));
  });
  ...
  document.querySelectorAll('.rss-shortcodes code').forEach(code => code.addEventListener('click', () => {
    const start = template.selectionStart;
    template.value = template.value.slice(0, start) + code.dataset.code + template.value.slice(template.selectionEnd);
    template.focus();
    template.selectionStart = template.selectionEnd = start + code.dataset.code.length;
  }));
})();
</script>
